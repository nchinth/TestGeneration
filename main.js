...
var mockFileLibrary = 
{
	pathExistsWithFiles:
	{
		'path/fileExists': {'some-file.txt': 'file content here'}
	},
	pathExists:
	{
		'path/fileExists': {}
...
function generateTestCases()
{
	...
	for ( var funcName in functionConstraints )
	{
		...
		for (var i =0; i < functionConstraints[funcName].params.length; i++ )
		{
			...
			if(paramName.indexOf('phoneNumber')> -1){
				isPhone = true;
		...
		// Generate Tests for individual params.
		...
		{
			var constraint = constraints[c];
			if( params.hasOwnProperty( constraint.ident ) && constraint.kind == "integer")
			{
				if(constraint.operator == "<"){
					params[constraint.ident] = constraint.value+1;
				}
			...
			traverse(node, function(child)
			{
				if( child.type === 'BinaryExpression' && child.operator == "==")
				{
					...
					if( child.left.type == 'Identifier' && child.left.name == "area"){
						...
						var phoneNumber = '\''+rightHand+'123123'+'\'';
						functionConstraints[funcName].constraints.push( 
							new Constraint(
							{
								ident: 'phoneNumber',
						...
						rightHand = parseInt(rightHand) + 1;
						phoneNumber = '\''+rightHand+'123123'+'\'';
						functionConstraints[funcName].constraints.push( 
							new Constraint(
							{
								ident: 'phoneNumber',
					...
					//console.log(child.left.callee.property.name);
					if(child.left.type == "CallExpression" && child.left.callee.property &&
					 child.left.callee.property.name =="indexOf" )
					{
						var ident_name = child.left.callee.object.name;
						functionConstraints[funcName].constraints.push( 
						new Constraint(
							{
								ident: ident_name,
				...
				if( child.type === 'BinaryExpression' && child.operator == ">")
				{
					if( child.left.type == 'Identifier' && params.indexOf( child.left.name ) > -1)
					{
						// get expression from original source code:
						var expression = buf.substring(child.range[0], child.range[1]);
						var rightHand = parseInt(buf.substring(child.right.range[0], child.right.range[1])) + 1;
				...
				if( child.type === 'BinaryExpression' && child.operator == "!=")
				{
					if( child.left.type == 'Identifier' && params.indexOf( child.left.name ) > -1)
					{
						...
						rightHand = '\'not'+buf.substring(child.right.range[0]+1, child.right.range[1]-1)+'\'' ;
